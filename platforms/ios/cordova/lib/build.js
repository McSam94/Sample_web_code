    ...
    return require('./list-devices')
        ...
        .then(function () {
            ...
            if (!buildOpts.device) {
                ...
                var promise = require('./list-emulator-build-targets').targetForSimIdentifier(
                    newTarget,
                );
                return promise.then(function (theTarget) {
                    if (!theTarget) {
                        return getDefaultSimulatorTarget().then(function (defaultTarget) {
                            ...
                            events.emit(
                                'warn',
                                `No simulator found for "${newTarget}. Falling back to the default target.`,
                            );
                            events.emit(
                                'log',
                                `Building for "${emulatorTarget}" Simulator (${defaultTarget.identifier}, ${defaultTarget.simIdentifier}).`,
                            );
        ...
        .then(function (name) {
            ...
            if (buildOpts.codeSignIdentity) {
                ...
                extraConfig +=
                    'CODE_SIGN_IDENTITY[sdk=iphoneos*] = ' + buildOpts.codeSignIdentity + '\n';
            }
            ...
            if (buildOpts.provisioningProfile) {
                ...
                events.emit(
                    'verbose',
                    'ProvisioningProfile build option NOT set, changing project settings to Automatic.',
                );
            ...
            return Q.nfcall(
                fs.writeFile,
                path.join(__dirname, '..', 'build-extras.xcconfig'),
                extraConfig,
                'utf-8',
            );
        })
        .then(function () {
            ...
            return superspawn.spawn('xcodebuild', xcodebuildArgs, {
                cwd: projectPath,
                printCommand: true,
                stdio: 'inherit',
            });
...
function findXCodeProjectIn(projectPath) {
    ...
    if (xcodeProjFiles.length > 1) {
        events.emit(
            'warn',
            'Found multiple .xcodeproj directories in \n' + projectPath + '\nUsing first one',
        );
    }
...
function getXcodeBuildArgs(
    ...
    if (isDevice) {
        ...
        if (customArgs.archivePath) {
            customArgs.otherFlags = customArgs.otherFlags.concat([
                '-archivePath',
                customArgs.archivePath,
            ]);
        }
    }
