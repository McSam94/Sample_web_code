...
var handlers = {
    ...
    framework: {
        ...
        install: function (obj, plugin, project, options) {
                ...
                if (keepFrameworks.indexOf(src) < 0) {
                    if (obj.type === 'podspec') {
                        ...
                        events.emit(
                            'verbose',
                            util.format(
                                'Adding non-custom framework to project... %s -> %s',
                                src,
                                JSON.stringify(opt),
                            ),
                        );
                        ...
                        events.emit(
                            'verbose',
                            util.format(
                                'Non-custom framework added to project. %s -> %s',
                                src,
                                JSON.stringify(opt),
                            ),
                        );
            ...
            var existsEmbedFrameworks = project.xcode.buildPhaseObject(
            ...
            events.emit(
                'verbose',
                util.format(
                    'Adding custom framework to project... %s -> %s',
                    src,
                    JSON.stringify(opt),
                ),
            );
            ...
            events.emit(
                'verbose',
                util.format(
                    'Custom framework added to project. %s -> %s',
                    src,
                    JSON.stringify(opt),
                ),
            );
        ...
        uninstall: function (obj, plugin, project, options) {
            ...
            var targetDir = fixPathSep(
                path.resolve(project.plugins_dir, plugin.id, path.basename(src)),
            );
    ...
    'js-module': {
        install: function (obj, plugin, project, options) {
            ...
            var moduleName =
                plugin.id + '.' + (obj.name || path.basename(obj.src, path.extname(obj.src)));
            ...
            if (options && options.usePlatformWww) {
                var platformWwwDestination = path.resolve(
                    project.platformWww,
                    'plugins',
                    plugin.id,
                    obj.src,
...
function installHelper(type, obj, plugin_dir, project_dir, plugin_id, options, project) {
    ...
    if (link) {
        ...
        project_ref =
            'Plugins/' + fixPathSep(path.relative(fs.realpathSync(project.plugins_dir), trueSrc));
    } else {
        if (options && options.force) {
    ...
    if (type === 'header-file') {
        ...
        project.xcode.addSourceFile(
            project_ref,
            obj.compilerFlags ? { compilerFlags: obj.compilerFlags } : {},
        );
    }
}
