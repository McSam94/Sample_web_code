    ...
    return Q()
            ...
            if (!install_target) {
                ...
                return device.list().then(function (device_list) {
                    if (device_list.length > 0) {
                        events.emit(
                            'warn',
                            "No target specified, deploying to device '" + device_list[0] + "'.",
                        );
            ...
            return device.list().then(function (devices) {
                ...
                return emulator.list_started().then(function (started_emulators) {
                    ...
                    return emulator.list_images().then(function (avds) {
                        ...
                        return Q.reject(
                            "Target '" + install_target + "' not found, unable to run project",
                        );
                    });
                });
            });
        ...
        .then(function (resolvedTarget) {
            return new Promise((resolve) => {
                ...
                const buildOptions = require('./build').parseBuildOptions(
                    runOptions,
                    null,
                    self.root,
                );
